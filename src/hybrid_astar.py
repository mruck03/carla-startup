...
class C:  # Parameter config
    ...
    RF = 3.743  # [m] distance from rear to vehicle front end of vehicle (Tesla Model 3)
    RB = 0.977  # [m] distance from rear to vehicle back end of vehicle (unchanged)
    W = 1.849  # [m] width of vehicle (Tesla Model 3)
    WD = 1.584  # [m] distance between left-right wheels (unchanged, assuming similar to general vehicles)
...
def extract_path(closed, ngoal, nstart):
    ...
    direc[0] = direc[1]
    path = Path_astar(rx, ry, ryaw, direc, cost)
...
def calc_parameters(sx, sy, gx, gy, ox, oy, xyreso, yawreso, kdtree):
    minx = round(min(min(ox), sx-5, gx-5) / xyreso)
    miny = round(min(min(oy), sy-5, gy-5) / xyreso)
    maxx = round(max(max(ox), sx+5, gx+5) / xyreso)
    maxy = round(max(max(oy), sy+5, gy+5) / xyreso)
